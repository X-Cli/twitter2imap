...
class IMAP4_SSL_CHECK(IMAP4_SSL):
        def __init__(self, host = '', port = IMAP4_SSL_PORT, keyfile = None, certfile = None, ca_certs = None):
            self.keyfile = keyfile
            self.certfile = certfile
        ...
        def open(self, host = '', port = IMAP4_SSL_PORT):
            ...
            if self.cacerts is not None:
                self.sslobj = ssl.wrap_socket(self.sock, self.keyfile, self.certfile, ca_certs=self.cacerts, cert_reqs=ssl.CERT_REQUIRED)
            else:
                self.sslobj = ssl.wrap_socket(self.sock, self.keyfile, self.certfile)
            self.file = self.sslobj.makefile('rb')
